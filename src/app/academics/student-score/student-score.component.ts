...
export class StudentScoreComponent implements OnInit {
  ...
  getExamSeriesScore(){
    this.initMarkList();
    this.mark_list = this.mark_list.filter(x=>x.current_class_key === this.selected_class_info_key);
    if(this.selected_division!=='All'){
      this.mark_list = this.mark_list.filter(x=>x.division === this.selected_division);
    }
    this.isEditing = false;
    this.has_constituent_subjects = false;
    ...
    this.studentScoreService.getStudentScoreForExam(exam.exam_key).then(resp => {
      if(resp.length>0){
          for(let student_score of resp){
            let student_detail = this.mark_list.find(x => x.student_key === student_score.student_key);
  ...
  calculateFinalMark(){
    ...
    let calculation_method = this.selected_exam.aggregation_info.aggregation_method.aggregate_type;
    for(let student of selected_students){
      student.exam_key = student.exam_key;
